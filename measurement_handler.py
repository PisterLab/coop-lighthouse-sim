...
class MeasurementHandler:
	...
	def compute_lighthouse_meas(self, unknown_anchor):
		...
		x_column_prev = np.array([l.posHist[-2][0] for l in lighthouse_drones])[:,None]
		y_column_prev = np.array([l.posHist[-2][1] for l in lighthouse_drones])[:,None]
		...
		phi_robot_vec_k = np.array([(l._att + PI) % (2 * PI) - PI for l in lighthouse_drones])[:,None]

		phi_robot_vec_prev = np.array([(l.attHist[-2] + PI) % (2 * PI) - PI for l in lighthouse_drones])[:,None]
	...
	def compute_anchor_meas(self, lighthouse_drone):
	    ...
	    x_column = [a._pos[0] for a in self._anchor_drones]
	    y_column = [a._pos[1] for a in self._anchor_drones]
	    ...
	    # find a phi that matches current
	    phi_robot_k = (lighthouse_drone._att + PI) % (2 * PI) - PI
